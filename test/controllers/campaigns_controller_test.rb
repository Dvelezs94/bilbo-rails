...
class CampaignsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "copy budget campaign" do
    ...
    post create_copy_campaign_path(@campaign), params: {campaign: {name: "CopyCampaign 1", starts_at: Time.zone.now, ends_at: Time.zone.now + 24.hours} }
    #get the new record (the one we created in the previous line)
    campaign_2 = Campaign.find_by(name: "CopyCampaign 1")
    attributes_1 = @campaign.attributes.delete_if{|key, value| !["project_id", "budget", "ad_id", "provider_campaign", "classification", "objective"].include?(key)}
  ...
  test "copy campaign per minute" do
    ...
    post create_copy_campaign_path(@campaign), params: {campaign: {name: "CopyCampaign 2", starts_at: Time.zone.now, ends_at: Time.zone.now + 24.hours} }
    #get the new record (the one we created in the previous line)
    campaign_2 = Campaign.find_by(name: "CopyCampaign 2")
    attributes_1 = @campaign.attributes.delete_if{|key, value| !["project_id", "imp", "minutes", "ad_id", "provider_campaign", "classification", "objective"].include?(key)}
    attributes_1.merge({boards: @campaign.boards.pluck(:id).sort})
    attributes_2 = campaign_2.attributes.delete_if{|key, value| !["project_id", "imp", "minutes", "ad_id", "provider_campaign", "classification", "objective"].include?(key)}
    attributes_2.merge({boards: campaign_2.boards.pluck(:id).sort})
    assert_equal attributes_1, attributes_2
  end
  ...
  test "copy campaign per hour" do
    ...
    post create_copy_campaign_path(@campaign), params: {campaign: {name: "CopyCampaign 3", starts_at: Time.zone.now, ends_at: Time.zone.now + 24.hours} }
    #get the new record (the one we created in the previous line)
  ...
  test "differences in copies" do
    @campaign = create(:campaign, name: "budget campaign", project: @user.projects.first, classification: 0, provider_campaign: @user.is_provider?)
    post create_copy_campaign_path(@campaign), params: {campaign: {name: "CopyCampaign 4", starts_at: Time.zone.now, ends_at: Time.zone.now + 24.hours} }
    #get the new record (the one we created in the previous line)
    ...
    different_dates = @campaign.starts_at != campaign_2.starts_at
  ...
  test "copy campaign with contents" do
    ...
    @boards_campaigns = create(:boards_campaigns, campaign_id: @campaign.id , board_id: @board.id, status: 1)
    @content_board_campaign = create(:contents_board_campaign, content_id: @content.id, boards_campaigns_id: @boards_campaigns.id)
    post create_copy_campaign_path(@campaign), params: {campaign: {name: "CopyCampaign", starts_at: Time.zone.now, ends_at: Time.zone.now + 24.hours} }
    #get the new record (the one we created in the previous line)
    campaign_2 = Campaign.find_by_name("CopyCampaign")
    attributes_1 = @campaign.attributes.delete_if{|key, value| !["project_id", "budget", "ad_id", "provider_campaign", "classification", "objective"].include?(key)}
    attributes_1.merge({boards: @campaign.boards.pluck(:id).sort})
    attributes_2 = campaign_2.attributes.delete_if{|key, value| !["project_id", "budget", "ad_id", "provider_campaign", "classification", "objective"].include?(key)}
    attributes_2.merge({boards: campaign_2.boards.pluck(:id).sort})
