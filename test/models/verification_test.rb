...
class VerificationTest < ActiveSupport::TestCase
  ...
  test "active campaign when user is not verified" do
    ...
    @camp1 = create(:campaign, name: "on", project_id: @user.projects.last.id, description: nil, state: false, ad_id: @ad.id, budget: 100, clasification: "budget", boards: [@board], budget: 350, state_updated_at: nil, status: "active", provider_campaign: @user.is_provider?)
