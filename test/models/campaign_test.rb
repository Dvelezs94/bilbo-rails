...
class CampaignTest < ActiveSupport::TestCase
  ...
  test "create per minute campaign" do
    @campaign = create(:campaign, name: "budget campaign", project: @user.projects.first, classification: 1, provider_campaign: @user.is_provider?)
    assert_equal "per_minute", @campaign.classification
  end
  test "create per hour campaign" do
    @campaign = create(:campaign, name: "budget campaign", project: @user.projects.first, classification: 2, provider_campaign: @user.is_provider? )
  ...
  test "budget campaign has correct fields" do
    @budget = "500"
    @campaign = create(:campaign, name: "budget campaign", project: @user.projects.first, classification: 0, budget: @budget, provider_campaign: @user.is_provider?)
      assert @budget, @campaign.budget
  end
  test "per minute campaign has correct fields" do
    @imp = 5
    @minutes = 5
    @campaign = create(:campaign, name: "budget campaign", project: @user.projects.first, classification: 1, imp: @imp, minutes: @minutes, provider_campaign: @user.is_provider?)
