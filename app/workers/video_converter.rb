  ...
  def perform(ad_id,video_id)
    ...
    begin
      ...
      if movie.duration > 10
        ...
        system('ffmpeg', '-probesize', "100M", '-analyzeduration', "100M", '-i', cut_video_tmpfile, '-compression_level', "6", '-q:v', "60", '-preset', "picture", '-loop', "0", '-vf', "fps=20, scale=720:-1", webp_video_tmpfile)
        File.delete(cut_video_tmpfile)
      else
          system('ffmpeg', '-probesize', "100M", '-analyzeduration', "100M", '-i', orig_video_tmpfile, '-compression_level', "6", '-q:v', "60", '-preset', "picture", '-loop', "0", '-vf', "fps=20, scale=720:-1", webp_video_tmpfile)
      end
