class ChartsController < ApplicationController
  ...
  def daily_impressions
    ...
    render json: @campaign.boards.map{|board| {name: board.name, data: board.impressions.where(campaign_id: @campaign, created_at: params[:start_date]..params[:end_date]).group_by_day(:created_at, format: "%b %d").count}}
  end
