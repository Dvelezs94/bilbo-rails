class Admin::BoardActionsController < ApplicationController
  ...
  def provider_statistics
    ...
    @start_date = (params[:select][:year]+"-" +params[:select][:month]+"-"+Date.today.day.to_s).to_datetime.beginning_of_month + 25 rescue Date.today.beginning_of_month + 25
    @end_date = @start_date.beginning_of_month - 1.month + 24
