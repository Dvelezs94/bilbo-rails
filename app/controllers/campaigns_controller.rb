class CampaignsController < ApplicationController
  ...
  def provider_index
    if params[:q] == "review"
      @board_campaigns = BoardsCampaigns.where(board_id: @project.boards.pluck(:id), campaign_id: Campaign.all.active.pluck(:id)).in_review
    elsif params[:bilbo].present?
      @board_campaigns = BoardsCampaigns.where(board_id: @project.boards.where(name: params[:bilbo]), campaign_id: Campaign.all.active.pluck(:id)).approved
    else
      @board_campaigns = BoardsCampaigns.where(board_id: @project.boards.pluck(:id), campaign_id: Campaign.all.active.pluck(:id)).approved
    end
  end
